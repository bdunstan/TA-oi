...
def write_index( helper, ew, company_key ):
    ...
    for filename in os.listdir( f'{TA_OI_PATH}/data/{company_key}' ):
        ...
        if sourcetype:
            ...
            helper.log_info(f'process: {TA_OI_PATH}/data/{company_key}/{filename}')
            with open (f'{TA_OI_PATH}/data/{company_key}/{filename}', 'r') as source:
                for line in source:
                    ...
                    if 'timestamp' in jline:
                        ...
                        event = helper.new_event(line, time=jline['timestamp'], host=None, index=helper.get_output_index(), source=None, sourcetype=f'cisco::bcs::{sourcetype}', done=True, unbroken=True)
                        helper.log_info(f'event: {line}')
                        return
